NxtMnthPred <- function(data.frame){
  library(MASS)
  library(neuralnet)
  ReqError <- 0.1
  ...
  for (i in 1:(Int.No.Row-20)){
    data.frame$ndc[i] <- data.frame$Close[i+20]
    data.frame$Predicted.For[i] <- data.frame$Date[i+20]
  }
  Data <- data.frame
  Data<- na.omit(Data)
  ...
  for (k in 1:5) {
    neuralModel<-neuralnet(ndc~Open+High+Low+Close+AdjClose,hidden=c(k),linear.output=T,data=trainD, stepmax = 100000)
    plot(neuralModel)
    ...
    if (error < ReqError){
      PredTable <- data.frame(Predicted.On=Data$Date[(No.Row-49):No.Row], Predicted.For=Data$Predicted.For[(No.Row-49):No.Row] , ActualValue=test$ndc, PredictedValue=predV, Error=((abs((test$ndc-predV))/(test$ndc))*100))
      ReqError <- error
    }
  }
  for(i in 1:5){
    for(j in 1:5){
      neuralModel<-neuralnet(ndc~Open+High+Low+Close+AdjClose,hidden=c(j,i),linear.output=T,data=trainD)
      plot(neuralModel)
      predictions<-compute(neuralModel,testD[,-1])
      predV <- (predictions$net.result*(maxndc-minndc)+minndc)
      error <- (sum(abs((test$ndc-predV))/(test$ndc))/nrow(test))
      if (error < ReqError){
        PredTable <- data.frame(Predicted.On=Data$Date[(No.Row-49):No.Row], Predicted.For=Data$Predicted.For[(No.Row-49):No.Row] , NxtMnth.ActualValue=test$ndc, PredictedValue=predV, Error=((abs((test$ndc-predV))/(test$ndc))*100))
        ReqError <- error
      }
    }
    if (error < ReqError){
